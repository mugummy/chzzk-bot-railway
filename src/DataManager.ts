...
export class DataManager {
    ...
    static async loadData(channelId: string): Promise<any> {
        console.log(`[DataManager] 📥 Loading data for: ${channelId}`);
    ...
    private static async executeActualSave(channelId: string, data: any) {
        try {
            ...
            console.log(`[DataManager] 💾 Saved state for ${channelId}`);
        } catch (e) { 
    ...
    private static getDefault(channelId: string) { 
        return { 
            ...
            songQueue: [], currentSong: null, votes: [], 
            roulette: { items: [], winner: null },
            draw: { isActive: false, candidates: [] },
            participants: { queue: [], active: [], isActive: false, max: 10 }, 
            commands: [], macros: [], counters: [], points: {}, ranking: []
        }; 
    }
}
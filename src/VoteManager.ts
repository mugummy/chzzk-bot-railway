...
export interface VoteOption { id: string; text: string; }
export interface Voter { userIdHash: string; nickname: string; optionId: string; }
...
export class VoteManager {
    ...
    public setCurrentVote(vote: VoteSession) { this.currentVote = vote; }
    ...
    public async endVote() {
        if (this.currentVote) {
            ...
            if (this.bot.chat?.connected) this.bot.chat.sendChat(`📊 투표 종료! 총 ${this.currentVote.totalVotes}표`);
            this.currentVote = null;
            this.notify();
        }
    }

    public resetVote() { this.currentVote = null; this.notify(); }
    public deleteHistory(voteId: string) { this.voteHistory = this.voteHistory.filter(v => v.id !== voteId); this.notify(); }
    ...
    public async handleChat(chat: ChatEvent) {
        ...
        if (!isNaN(optionIndex) && this.currentVote.options[optionIndex]) {
            ...
            if (!this.currentVote.voters.some(v => v.userIdHash === userId)) {
                ...
                this.currentVote.voters.push({ userIdHash: userId, nickname: chat.profile.nickname, optionId: optionId });
                this.notify();
            }
        }
    }

    public getState() { return { currentVote: this.currentVote, history: this.voteHistory }; }
    public getVoters(voteId?: string) {
        if (!voteId && this.currentVote) return this.currentVote.voters;
        return voteId ? (this.voteHistory.find(v => v.id === voteId)?.voters || []) : [];
    }
}

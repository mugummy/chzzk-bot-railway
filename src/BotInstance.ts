...
export class BotInstance {
    ...
    public async setup() {
        ...
        // [수정] 채팅 활성화/비활성화 시 공지 메시지 전송 로직
        let prevChatEnabled = this.settings.getSettings().chatEnabled;
        this.settings.setOnStateChangeListener(() => {
            const currentSettings = this.settings.getSettings();
            
            // 상태가 변경되었을 때만 메시지 전송
            if (this.isLoggedIn && this.chat && prevChatEnabled !== currentSettings.chatEnabled) {
                if (currentSettings.chatEnabled) {
                    this.chat.sendChat('🟢 gummybot 응답 기능이 활성화되었습니다.');
                } else {
                    this.chat.sendChat('🔴 gummybot 응답 기능이 비활성화되었습니다.');
                }
                prevChatEnabled = currentSettings.chatEnabled;
            }
            
            this.notify('settingsUpdate', currentSettings);
            this.saveAll(); // 설정 변경 즉시 저장
        });
        ...
        try {
            ...
            this.livePollingTimer = setInterval(async () => {
                try {
                    ...
                    if (latestLive) this.liveDetail = latestLive;
                } catch (e) {}
            }, 10000);
        ...
        } catch (err) { console.error('[BotInstance] Setup Error:', err); }
    }

    ...
    ws.on('message', async (message) => {
        try {
            ...
            switch (data.type) {
                ...
                // [수정] 개별 항목 토글 핸들러 추가
                case 'toggleCommand': 
                    const cmd = bot.commands.getCommands().find(c => (c.triggers?.[0] || (c as any).trigger) === data.data.trigger);
                    if (cmd) { cmd.enabled = data.data.enabled; bot.saveAll(); }
                    break;
                case 'toggleCounter':
                    const cnt = bot.counters.getCounters().find(c => c.trigger === data.data.trigger);
                    if (cnt) { cnt.enabled = data.data.enabled; bot.saveAll(); }
                    break;
                case 'toggleMacro':
                    const mac = bot.macros.getMacros().find(m => m.id === data.data.id);
                    if (mac) { mac.enabled = data.data.enabled; bot.saveAll(); }
                    break;
